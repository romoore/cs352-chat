...
public class ChatClient extends Thread implements MessageListener,
   ...
   */
  public static final String LICENSE_NOTIFICATION = "CS352 Example Chat Client version 1.0.5\n"
  ...
  /**
   * Parse command-line arguments and start a single instance of the ChatClient
   * class.
   * 
   * @param args
  ...
  public static void main(String[] args) {
    ...
    /*
     * If the remote host and port were provided, try to connect. Shouldn't be
     * fatal if it fails.
     */
    if (args.length >= 3) {
   ...
   * client is not matched or the old connection is closed, then it will connect
   * to the client.
   * 
   * @param remoteHost
  ...
  protected Client makeClient(String remoteHost, int port, String username) {
    Client newClient = new Client(remoteHost, port, username, this.username,
        this.listenPort);
    return newClient;
  }
  ...
  public void clientMessageArrived(final Client client,
    ...
    this.workers.execute(new Runnable() {
      @Override
      public void run() {
        ChatClient.this.addClient(message.getIpAddress(), message.getPort(),
            message.getUsername());
      }
    });
  ...
  /**
   * Deregisters the client from the local client, disconnects it, and notifies
   * the user interface.
   */
  @Override
  public void disconnectMessageArrived(final Client client) {
    this.workers.execute(new Runnable() {
      ...
      public void run() {
        ...
        ChatClient.this.userInterface.clientDisconnected("User quit.", client);
      }
    });
  ...
  public void run() {
    ...
    while (this.keepRunning) {
      try {
        ...
        /*
         * Pass the actual work of adding the client to another thread, freeing
         * this thread to accept new clients.
         */
        this.workers.execute(new Runnable() {
  ...
  public void broadcastChatMessage(final String input) {
    this.workers.execute(new Runnable() {
      @Override
      public void run() {
        for (Iterator<Client> clientIter = ChatClient.this.clients.iterator(); clientIter
            .hasNext();) {
          Client client = clientIter.next();
          try {
    ...
    this.workers.execute(new Runnable() {
      ...
      public void run() {
        try {
          ...
          client.disconnect();
          ChatClient.this.userInterface.clientDisconnected(
              "Failed to send private chat message/" + e.getMessage(), client);
        }
