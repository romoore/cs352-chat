 ...
 * 
 * <ul>
 *   <li>After a connection is established, both sides transmit a Handshake message with
 *       their respective listen ports and usernames.</li>
 *   <li>If the handshakes are validated correctly, Chat, ClientExchange, Keep-Alive,
 *       and Disconnect messages may be exchanged at any time.</li>
 *   <li>Chat messages are sent any time the user provides a message to be broadcast
 *       or sent to a specific client.</li>
 *   <li>ClientExchange messages are sent to currently-connected remote clients after a new
 *       client has connected to the listen socket of the local client.  Clients receiving a
 *       ClientExchange message may decide whether or not to connect to the new client.</li>
 *   <li>Disconnect messages should be sent when a client decides to exit the chat.  While the
 *       clients handle unexpected socket closure gracefully, it is best practice to send a 
 *       Disconnect message before closing a connection.  Clients receiving a Disconnect message
 *       should assume that the remote side of the socket is closed.</li>
 *   <li>Keep-Alive messages are usually sent to validate a connection to a remote client if
 *       a ClientExchange message has been received with the same information, or if a client
 *       connects from the same IP and has the same listen port value.</li>
 * </ul>
 * 
 * The messages have a standard header of a 4-byte unsigned integer length field and a 1-byte 
 * message type value.  The message type values are listed below:
 * 
 * <ul>
...
public abstract class AbstractMessage {
	...
	/**
	 * Static reference to a disconnect message so that new objects don't need
	 * to be allocated.
	 */
	public static final DisconnectMessage DISCONNECT_MESSAGE = new DisconnectMessage();

	/**
	 * Static reference to a keep-alive message so that new objects don't need
	 * to be allocated.
	 */
	...
	/**
	 * Creates a new abstract message with the specified length and message
	 * type.
	 * 
	 * @param length
	 ...
	 * @param out the OutputStream on which to write the message.
	 * @throws IOException if an IOException is thrown by the OutputStream.
	 */
	public static void encodeMessage(final AbstractMessage message,
		...
		if (message.getLength() > 1) {
			...
			case AbstractMessage.TYPE_HANDSHAKE_MESSAGE:
				...
				dout.write(HandshakeMessage.PROTOCOL_STRING
						.getBytes("UTF-16BE"));
	...
	/**
	 * Decodes and returns the next message from the provided InputStream.  If no message can be decoded,
	 * returns null.
	 * @param in the InputStream from which to decode the next message.
	 * @return the decoded message, or null if no message can be decoded.
	 * @throws IOException if the socket is closed or an IOException is thrown by the InputStream.
	 */
	public static AbstractMessage decodeMessage(final InputStream in)
		...
		case AbstractMessage.TYPE_CLIENT_EXCHANGE_MESSAGE:
			...
			message = new ClientExchangeMessage(addx.getHostAddress(), port,
					uname);
			break;
		case AbstractMessage.TYPE_HANDSHAKE_MESSAGE:
			...
			if (!protocolString.equals(HandshakeMessage.PROTOCOL_STRING)) {
				System.err
						.println("Received invalid handshake protocol string: "
								+ protocolString);
			}
			...
			byte[] nameBytes = new byte[messageLength
					- 3
